...
export default function WindowsWorkspace() {
  ...
  const loadDesktop = useCallback(async () => {
    setUiError(null);
    try {
      const { nodes } = await apiJson<{ nodes: FsNode[] }>("/api/fs");
  ...
  useEffect(() => {
    // restore theme
    try {
      const storedId = window.localStorage.getItem(LS_THEME_ID) as WindowsThemeId | null;
      if (storedId) setThemeId(storedId);
  ...
  async function deleteFromDesktop(node: FsNode) {
    ...
    setUiError(null);
    try {
      await apiJson(`/api/fs/${node.id}`, { method: "DELETE" });
      await loadDesktop();
    } catch (e) {
      setUiError(e instanceof Error ? e.message : "Operation failed");
    } finally {
