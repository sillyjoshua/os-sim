...
import OssScriptApp from "@/components/os/OssScriptApp";
import { createNode, deleteNode, listChildren } from "@/lib/local-fs";
import { isOssFile } from "@/lib/oss-script";

type AppKind = "explorer" | "notepad" | "oss" | "settings";
...
export default function WindowsWorkspace() {
  ...
  const openOssForNode = useCallback(
    (node: FsNode) => {
      openWindow("oss", {
        title: `OSS Runner — ${node.name}`,
        w: 760,
        h: 560,
        data: { fileId: node.id },
      });
  ...
  const openFileForNode = useCallback(
    (node: FsNode) => {
      if (isOssFile(node)) {
        openOssForNode(node);
      } else {
    ...
    [openNotepadForNode, openOssForNode],
  );

  const getDocumentsFolderId = useCallback(() => {
    const root = listChildren(null);
    const docs = root.find(
      (n) => n.type === "folder" && n.name.toLowerCase() === "documents",
    );
    return docs?.id ?? null;
  }, []);

  const openOssTutorial = useCallback(() => {
    const docsId = getDocumentsFolderId();
    const children = listChildren(docsId);
    const tut = children.find(
      (n) => n.type === "file" && n.name.toLowerCase() === "oss tutorial.oss",
    );
    if (!tut) {
      setUiError("OSS Tutorial.oss not found in Documents");
      return;
    }
    openOssForNode(tut);
  }, [getDocumentsFolderId, openOssForNode]);

  const newOssScriptInDocuments = useCallback(() => {
    const docsId = getDocumentsFolderId();
    const name = window.prompt("Script name", "My Script.oss");
    if (!name) return;
        ...
        >
          {w.app === "explorer" ? (
            <ExplorerApp
              ...
              onOpenFile={(node) => openFileForNode(node)}
            />
          ) : null}
          ...
          {w.app === "oss" ? (
            <OssScriptApp
              fileId={w.data.fileId as number}
              onSetTheme={(id) => {
                setThemeId(id);
              }}
            />
        ...
        {startOpen ? (
          ...
          >
            ...
            <div className="grid gap-2">
              ...
              <StartItem
                label="OSS Tutorial"
                sub="Learn the .oss scripting language"
                onClick={() => {
                  setStartOpen(false);
                  openOssTutorial();
                }}
              />
              <StartItem
                label="New OSS Script"
                sub="Create a new .oss file in Documents"
                onClick={() => {
                  setStartOpen(false);
                  newOssScriptInDocuments();
                }}
              />
